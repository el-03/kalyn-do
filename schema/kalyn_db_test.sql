-- SCHEMA
CREATE SCHEMA IF NOT EXISTS kalyn_db_test;

-- COLOR
CREATE TABLE IF NOT EXISTS kalyn_db_test.color
(
...
-- CATEGORY
CREATE TABLE IF NOT EXISTS kalyn_db_test.category
(
    id       SERIAL PRIMARY KEY,
...
-- ITEM NAME
CREATE TABLE IF NOT EXISTS kalyn_db_test.item_name
(
    id        SERIAL PRIMARY KEY,
...
-- ITEM DETAILS
CREATE TABLE IF NOT EXISTS kalyn_db_test.item_details
(
    id               SERIAL PRIMARY KEY,
    ...
    CONSTRAINT fk_item_details_item_name
        FOREIGN KEY (item_name_id) REFERENCES kalyn_db_test.item_name (id),

    CONSTRAINT fk_item_details_category
        FOREIGN KEY (category_id) REFERENCES kalyn_db_test.category (id),

    CONSTRAINT fk_item_details_color
        FOREIGN KEY (color_id) REFERENCES kalyn_db_test.color (id)
);

-- ITEM STOCKS (current stock snapshot)
CREATE TABLE IF NOT EXISTS kalyn_db_test.item_stocks
(
    id           SERIAL PRIMARY KEY,
    item_name_id INTEGER NOT NULL,
    category_id  INTEGER NOT NULL,
...
CREATE TABLE IF NOT EXISTS kalyn_db_test.item_stocks_log
(
    id           SERIAL PRIMARY KEY,
    item_name_id INTEGER NOT NULL,
    category_id  INTEGER NOT NULL,
...
CREATE OR REPLACE FUNCTION kalyn_db_test.update_item_stocks_snapshot()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
    INSERT INTO kalyn_db_test.item_stocks (
    ...
    DO UPDATE SET
        quantity = kalyn_db_test.item_stocks.quantity + EXCLUDED.quantity,
        updated_at = now();
...
ON kalyn_db_test.item_stocks_log;

CREATE TRIGGER trg_update_item_stocks_snapshot
AFTER INSERT ON kalyn_db_test.item_stocks_log
FOR EACH ROW
EXECUTE FUNCTION kalyn_db_test.update_item_stocks_snapshot();
